...
- **Data:** `useWorldData()` in `src/hooks/useWorldData.ts` — Convex queries → `WorldData` (`world`, `tileGrid`, `entities`, `assets`, `isLoading`). `IsometricGrid` calls it when `NEXT_PUBLIC_CONVEX_URL` is set; otherwise a static fallback grid.
- **Bootstrap:** `convex/seed.ts` `seedWorld` creates the world doc if missing and runs idempotent Phase 4 demo content once (`world.phase4DemoSeeded`). Client retries bootstrap when demo flag is unset (existing deployments).
